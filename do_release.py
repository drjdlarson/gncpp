...
def define_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Automatically create and push tag so CI/CD pipeline publishes a release")
...
def get_version() -> Tuple[int, int, int]:
    with open(VERSION_FILE, "r") as fin:
        for line in fin:
            matched = re.search("GNCPY_VERSION (\d+)(\d\d)(\d\d)", line)
            if matched:
    ...
    with open(VERSION_FILE, "r") as fin:
        with open(tmp_file, "w") as fout:
            for line in fin:
                matched = re.search("GNCPY_VERSION (\d+)(\d\d)(\d\d)", line)
                if matched:
                    ind = line.find("GNCPY_VERSION") + len("GNCPY_VERSION") + 1
                    new_line = line[:ind] + "{:02d}{:02d}{:02d}\n".format(major, minor, patch)
                    fout.write(new_line)
                else:
                    fout.write(line)
