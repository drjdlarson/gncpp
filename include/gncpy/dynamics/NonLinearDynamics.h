...
class NonLinearDynamics : public IDynamics<T> {
 ...
 public:
  matrix::Vector<T> propagateState(
      ...
      const StateTransParams* const stateTransParams = nullptr) const override;
  matrix::Vector<T> propagateState(
      T timestep, const matrix::Vector<T>& state,
  ...
  matrix::Matrix<T> getStateMat(
      T timestep, const StateTransParams* stateTransParams = nullptr) const;
  matrix::Matrix<T> getInputMat(
      T timestep, const ControlParams* controlParams = nullptr) const

      protected : inline matrix::Matrix<T> controlModel(
                      T timestep, const matrix::Vector<T>& state,
                      const matrix::Vector<T>& control,
                      const ControlParams* controlParams = nullptr) const {
    if (m_hasContolModel) {
