...
class Kalman : public IBayesFilter<T> {
 ...
 public:
  ...
  void setStateModel(std::shared_ptr<dynamics::IDynamics<T>> dynObj,
    ...
    if (procNoise.numRows() != dynObj->stateNames().size()) {
      throw exceptions::BadParams(
          "Process nosie size does not match they dynamics model dimension");
    }
  ...
  void setMeasurementModel(std::shared_ptr<measurements::IMeasModel<T>> measObj,
  ...
  inline std::shared_ptr<dynamics::IDynamics<T>> dynamicsModel()
      const override {
    if (m_dynObj) {
      return m_dynObj;
    } else {
  ...
  inline std::shared_ptr<measurements::IMeasModel<T>> measurementModel()
      const override {
    if (m_measObj) {
      return m_measObj;
    } else {
