...
class Matrix {
   ...
   public:
    ...
    explicit(false) operator Vector<T>() const {
        if (this->m_nRows == 1 || this->m_nCols == 1) {
            ...
            throw BadDimension(
                "Matrix dimensions do not allow casting to vector");
        }
    }
        ...
        for (size_t r = 0; r < m_nRows; r++) {
            for (size_t c = 0; c < rhs.m_nCols; c++) {
                ...
                for (size_t k = 0; k < m_nCols; k++) {
                    total += m_data[this->rowColToLin(r, k)] *
                             rhs.m_data[rhs.rowColToLin(k, c)];
                }
     ...
     * @return Matrix
     */
    Matrix operator()(size_t start_row, size_t start_col, size_t row_span,
                      size_t col_span) const {
    ...
    /**
     * @brief Calculate determinant of a matrix. Only square matrix implemented
     * currently
     *
     * @return T
     */
    ...
    /**
     * @brief Calculate the inverse of a matrix using LU linear solver. Only
     * square matrix implemented currently
     *
     * @return Matrix
     */
    ...
    inline T toScalar() const {
        if (this->size() != 1) {
            throw BadDimension(
                "Matrix has too many values to convert to scalar");
        }
   ...
   private:
    ...
    void serialize(Archive& ar) {
        ar(CEREAL_NVP(m_transposed), CEREAL_NVP(m_nRows), CEREAL_NVP(m_nCols),
           CEREAL_NVP(m_shape), CEREAL_NVP(m_strides), CEREAL_NVP(m_data));
    }

    inline bool isSquare() const { return m_nCols == m_nRows; }
    inline bool allowMultiplication(const Matrix& rhs) const {
        return numCols() == rhs.numRows();
    }
    inline bool isSameSize(const Matrix& rhs) const {
        return numRows() == rhs.numRows() && numCols() == rhs.numCols();
    }
    inline size_t rowColToLin(size_t r, size_t c) const {
        return m_transposed ? r + m_nRows * c : c + m_nCols * r;
    }
