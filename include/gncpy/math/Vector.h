...
class Vector final : public Matrix<T> {
   ...
   public:
    ...
    Vector<T> cross(const Vector& rhs) const {
        ...
        out[0] = static_cast<T>(this->operator()(1) * rhs(2) -
                                this->operator()(2) * rhs(1));
        out[1] = static_cast<T>(this->operator()(2) * rhs(0) -
                                this->operator()(0) * rhs(2));
        out[2] = static_cast<T>(this->operator()(0) * rhs(1) -
                                this->operator()(1) * rhs(0));
        return Vector<T>(out.size(), out);
    }
...
GNCPY_REGISTER_SERIALIZE_TYPES(lager::gncpy::matrix::Vector)
